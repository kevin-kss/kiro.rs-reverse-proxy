...
impl KiroProvider {
    ...
    /// 如果配置了 tls_proxy_url，则通过 Go TLS 代理发送请求
    fn base_url(&self, credentials: &KiroCredentials) -> String {
        let config = self.token_manager.config();
        if let Some(ref tls_proxy) = config.tls_proxy_url {
            // 通过 Go TLS 代理发送请求
            format!("{}/generateAssistantResponse", tls_proxy.trim_end_matches('/'))
        } else {
            format!(
    ...
    fn mcp_url(&self, credentials: &KiroCredentials) -> String {
        let config = self.token_manager.config();
        if let Some(ref tls_proxy) = config.tls_proxy_url {
            format!("{}/mcp", tls_proxy.trim_end_matches('/'))
        } else {
            format!(
